...
def _vllm_mm_k(
    ...
    for k in range(tl.cdiv(K, BK)):
        if EVEN_K:
            ...
            mask = k * BK + ko < K
            acc += tl.dot(
                tl.load(a, mask=mask[None, :], other=0.0),
...
def vllm_expand(x, weights, meta, base_output, BM=16, BN=64, BK=64, nw=4, ns=2):
    M, K = x.shape
    N = weights.shape[1]
    EVEN_K = K % BK == 0
    o = base_output
...
def make_our_bi(n, rank, n_unique, dev):
    ...
    class BI:
        ...
        lora_ranks = torch.tensor(
            [0] + [rank] * n_unique, dtype=torch.int32, device=dev
        )
...
def make_vllm_meta(n, rank, n_unique, n_slots, dev):
    ...
    slots = torch.tensor(
        [(i % n_unique) + 1 for i in range(n)], dtype=torch.int32, device=dev
    )
...
def header(title):
    ...
    print(f'{"="*80}')
    print(
        f'  {"n":>4}  {"n_uniq":>6}  {"seg":>8}  {"grp":>8}  {"grpv2":>8}  {"vllm":>8}  {"best":>6}'
    )
    print(f'  {"-"*58}')
...
def row(n, nu, ts, tg, tv2, tv):
    ...
    vals = [
        (t, nm)
        for t, nm in [(ts, "seg"), (tg, "grp"), (tv2, "v2"), (tv, "vllm")]
        if "n/a" not in str(t)
    ]
...
for rank, N in [(16, 4096), (64, 4096), (128, 4096), (64, 8192)]:
    ...
    for n in (8, 16, 32, 64, 128):
        for n_u in sorted({1, min(4, n), min(n, 8), n}):
            ...
            tg = (
                bench(lambda: lora_expand_decode_fwd(x, wo, bi, base_output=o.clone()))
                if use_grp
                else None
            )
            tv2 = (
                bench(
                    lambda: lora_expand_grouped_v2_fwd(x, wo, bi, base_output=o.clone())
                )
                if n_u > 0
                else None
            )
