...
def _expand_probe(
    ...
    seg_lens, seg_indptr, weight_indices, lora_ranks, scalings,
    ...
    x_ptrs = x + (seg_start + s_offset)[:, None] * x_stride_0 + k_offset[None, :] * x_stride_1
    ...
    for k in range(0, tl.cdiv(K_real, BLOCK_K)):
        ...
        x_tile  = tl.load(x_ptrs, mask=s_mask & (k_offset[None, :] < k_rem), other=0.0,
                           eviction_policy="evict_first")
        w_tile  = tl.load(w_ptrs, mask=(k_offset[:, None] < k_rem) & n_mask, other=0.0,
    ...
    out_ptr  = output + (seg_start + s_offset)[:, None] * output_stride_0 + n_offset[None, :] * output_stride_1
...
def run_probe(x, weights, output, bi, BLOCK_S, BLOCK_N, BLOCK_K, num_warps, num_stages):
    ...
    _expand_probe[grid](
        ...
        bi.seg_lens, bi.seg_indptr, bi.weight_indices, bi.lora_ranks, bi.scalings,
        BLOCK_S=BLOCK_S, BLOCK_N=BLOCK_N, BLOCK_K=BLOCK_K, num_warps=num_warps,
...
def sweep(n_segs: int, rank: int, N: int, label: str) -> None:
    ...
    print(f"  {'BLOCK_S':>7}  {'BLOCK_N':>7}  {'BLOCK_K':>7}  {'warps':>5}  {'stg':>3}  {'µs':>8}  {'BW%':>6}  {'K-iters':>8}")
    print(f"  {'-'*66}")
    ...
    for cfg in configs:
        ...
        try:
            ...
            if t_us < best_t:
                best_t = t_us
                best_cfg = cfg
            print(f"  {BS:>7}  {BN:>7}  {BK:>7}  {nw:>5}  {ns:>3}  {t_us:>7.1f}µ  {bw_pct:>5.1f}%  {k_iters:>8}{marker}")
    ...
    print(f"  Current autotune: {bench_cfg(lambda: run_probe(x, w, o.clone(), bi, 16, 64, 16, 8, 3))*1e6:.1f}µs")
